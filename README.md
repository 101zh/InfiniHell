# InfiniHell

Have fun trying to weave through bullets for the rest of time!

## Overview

This the repository for a bullet hell game

## Links
- [Itch](https://101zh.itch.io/infinihell) <- This is where you play the game!
- 101zh - [Github](https://github.com/101zh), [Itch](https://101zh.itch.io/)

## 3rd Party Assets/Libraries
- TextMeshPro
- Fonts by [TakWolf](https://github.com/TakWolf)
    - Ark Pixel Font - https://github.com/TakWolf/ark-pixel-font
    - Retro Pixel Font - https://github.com/TakWolf/retro-pixel-font

## Workspace
- Unity Version 2022.3.38f1
- Visual Studio or VS Code
- TextMeshPro
- New Input System
- Keyboard & Mouse

## Credits
Game development: 101zh
Sprites: 101zh
